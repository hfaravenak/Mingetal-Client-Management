               ...
               {OCClienteEntity.length > 0 ? (
                  OCClienteEntity.map((oc) => (
                     <SwiperSlide key={oc.id}>
                        <div
                           onClick={() => handleIdClick(oc.id_cliente)}
                           className="alerta"
                           style={
                              diferencia(oc) === 0
                                 ? { background: "#920000", border: "2px solid #5E0005", color: "#FFE6D9" }
                                 : diferencia(oc) === 1
                                 ? { background: "#E24747", border: "2px solid #840024", color: "#FFE4DE" }
                                 : diferencia(oc) === 2
                                 ? { background: "#E28247", border: "2px solid #AC3141", color: "#54433A" }
                                 : { background: "#FFC848", border: "2px solid #AC3141", color: "#4E4637" }
                           }
                        >
                           <div>
                              {" "}
                              {expandedId[oc.id_cliente] ? "-" : "+"} El cliente <b>{busquedaCliente(oc.id_cliente)}</b> debe pagar la orden de compra <b>{oc.id}</b>{" "}
                           </div>
                           {expandedId[oc.id_cliente] && (
                              <div className="expandible">
                                 {diferenciaValor(oc) < 0
                                    ? `Tiene un atraso de ${diferenciaValor(oc) * -1} días en el pago`
                                    : diferenciaValor(oc) > 0
                                    ? `Le quedan ${diferenciaValor(oc)} días para pagar`
                                    : "Debe pagar hoy"}
                              </div>
                           )}
                        </div>
